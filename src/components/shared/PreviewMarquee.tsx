  ...
  return (
    <div className="relative flex flex-col items-center group/marquee">
      ...
      >
        ...
        <div
          className={`flex items-center gap-2 px-6 py-2 bg-primary text-primary-foreground rounded-full shadow-lg font-bold text-[14px] z-20 transition-all hover:scale-105 active:scale-95 whitespace-nowrap border-b-2 border-primary-foreground/20 ${
            isVisible
              ? 'shadow-primary/60 ring-2 ring-primary/40'
              : 'shadow-primary/40'
          }`}
        >
          Preview
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="absolute top-[38px] left-1/2 -translate-x-1/2 w-[3px] bg-gradient-to-b from-primary to-primary/40 z-10 origin-top backdrop-blur-sm"
          />
        )}
      </AnimatePresence>
          ...
          >
            ...
            >
              {marqueeImages.map((src, index) => (
                <motion.div
                  key={`${src}-${index}`}
                  className="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden cursor-pointer relative transition-all duration-300 border border-white/5 hover:border-primary/50 shadow-lg group/img"
